...
public class UpdateUserTask {
    ...
    public UpdateUserListener mListener;
    private ArrayList<UpdateUser> updateUsers = new ArrayList<UpdateUser>();
    public UpdateUserTask(Context mContext, String requestString){
    ...
    public void doNetworkTask(){
        if (!mDialog.isDialogShowing())
            mDialog.ShowDialog();
        Util.postWithVolley(Consts.BASE_URL + Consts.UPDATE_USER_INFO, requestString, mContext, new VolleyCallback() {
            @Override
    ...
    private void parseResponse(String strResponse) {
        ...
        try {
            ...
            if(strResponse !="") {
                ...
                updateUser.setErrorCode(jObj.optInt("error_code"));
                updateUser.setMessage(jObj.optString("meesage"));
                updateUsers.add(updateUser);
                mListener.updateUserCallBack(updateUsers);
                if (mDialog.isDialogShowing())
